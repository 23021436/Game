...
Player::Player() :
    ...
    frameWidth(0), frameHeight(0), lane(1), scale(1.5f) {}  // Khởi tạo scale = 1.5f
...
bool Player::init(SDL_Renderer* renderer, const std::string& file) {
    ...
    lane = 1; // Làn giữa
    destRect = {
        laneX[lane] - scaledWidth / 2, // Căn giữa nhân vật
        600,
        scaledWidth,
        scaledHeight
    };
...
void Player::handleEvent(const SDL_Event& e) {
    if (e.type == SDL_KEYDOWN && e.key.repeat == 0) {
        ...
        int scaledWidth = static_cast<int>(frameWidth * scale);
        destRect.x = laneX[lane] - scaledWidth / 2; // Căn giữa nhân vật
    }
}
